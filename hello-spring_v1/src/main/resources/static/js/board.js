$().ready(function() {
    ...
    $(".search-button").on("click", function() {
        // /?pageNo=0&listSize=#list-size&searchType=#search-type값&searchKeyWord=#search-keyword값
        var pageNo = 0;
        var listSize = $("#list-size").val();
        var searchType = $("#search-type").val();
        var searchKeyword = $("#search-keyword").val();
    ...
    $(".attach-files").on("click", ".add-file", function() {
        //   $(".add-file").on("click", function () {  //아래 addbutton에 삭제기능 추가를 넣어야함으로(중복된 값 넣음)
        // 해당 코드를 통하여 이벤트를 주지 않고 부모에게 이벤트를 줘서 반복 없이 실행되게 한다
