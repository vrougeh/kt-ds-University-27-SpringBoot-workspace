...
public class SearchListVO {
	...
	// 총 몇개의 페이지가 생성되느냐
	// 게시글의 개수 / listSize 를 올림 처리
	private int pageCount;

	// 하나의 페이지 그룹에 보여줄 페이지의 게수
	private int pageCountInGroup;

	// 페이지 그룹의 게수 (pageCount / pageCountInGroup) 올림처리
	private int pageGroupCount;

	// 현재 노출되고 있는 페이지 번호가 속한 그룹의 번
	private int groupNo;
	...
	public void computePagination(int articleCount) {
		...
		// 현재페이지 그룹의 마지막 페이지 번호 계산
		// (현재 페이지 그룹번호 + 1) * 페이지 그룹당 페이지 개수
		...
		// 이전 그룹의 시작페이지 번호 계산
		// 현재 그룹의 시작 페이지 번호 - 페이지 그룹당 페이지 개수
